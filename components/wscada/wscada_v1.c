...
static const uint32_t PARAM_CODE_DISTANCE   = 101;

static esp_err_t _http_event_handler(esp_http_client_event_t *evt);
static void aes128_encrypt(const uint8_t *input, size_t length, uint8_t *output, const uint8_t *key);
static void aes128_decrypt(const uint8_t *input, size_t length, uint8_t *output, const uint8_t *key);
...
esp_err_t wscada_v1_post_lidar(uint32_t fat32_time, uint32_t interval, float distance)
{
    ...
    aes128_encrypt(unencrypted_buf + sizeof(uint32_t), encrypted_size, final_packet_buf + sizeof(uint32_t), raw_key);
    ...
    if (err == ESP_OK)
    {
        ...
        if (status_code == 200 && response_data.length >= 16)
        {
            ...
            aes128_decrypt(response_data.buffer, 16, decrypted_response, raw_key);
