...
# X-axis ticks every 20 cm
start = int(distance.min() // 20) * 20
end = int(np.ceil(distance.max() / 20)) * 20
plt.xticks(np.arange(start, end + 1, 20))

# Labels and title
plt.xlabel("BOSCH  distance (cm)", fontsize=12)
plt.ylabel("Error (cm)", fontsize=12)
plt.title("BOSCH Distance vs Error", fontsize=14)
